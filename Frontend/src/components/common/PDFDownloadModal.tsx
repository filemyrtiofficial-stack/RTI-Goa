...
import { BASE_URL_DISPLAY } from '../../config/api';
...
export const PDFDownloadModal: React.FC<PDFDownloadModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Get API base URL once - use same config as rest of app
      const apiBaseUrl = BASE_URL_DISPLAY;
      ...
      // Check if this is a Goa PDF (full public path) or other state (relative path)
      let state: string;
      let category: string;
      let filename: string;
      
      if (pdfPath.startsWith('/assets/PDF/RTI Templates For Goa State/')) {
        // Goa PDF - extract from full public path
        const goaPath = pdfPath.replace('/assets/PDF/RTI Templates For Goa State/', '');
        const goaParts = goaPath.split('/');
        state = 'goa';
        category = encodeURIComponent(goaParts[0]);
        filename = encodeURIComponent(goaParts[1]);
      } else {
        // Other states - extract from relative path (format: "state/category/filename.pdf")
        const pathParts = pdfPath.split('/');
        state = pathParts[0]; // e.g., "delhi" or "karnataka"
        category = encodeURIComponent(pathParts[1]);
        filename = encodeURIComponent(pathParts[2]);
      }
      ...
      try {
        // For Goa, backend might need special handling, but try standard format first
        // If backend doesn't support Goa, it will fail and fall back to public folder
        let apiUrl: string;
        if (state === 'goa') {
          // Try Goa-specific API path
          apiUrl = `${apiBaseUrl}/api/v1/pdf/goa/${category}/${filename}`;
        } else {
          apiUrl = `${apiBaseUrl}/api/v1/pdf/${state}/${category}/${filename}`;
        }
        ...
        if (response && response.ok) {
          ...
          // Create a link and open it in new tab to set proper filename
      ...
      if (!downloadSuccess) {
        try {
          ...
          // Handle Goa PDFs with special path structure
          let publicPath: string;
          if (state === 'goa') {
            publicPath = `/assets/PDF/RTI Templates For Goa State/${decodedCategory}/${decodedFilename}`;
          } else {
            publicPath = `/assets/PDF/${state}/${decodedCategory}/${decodedFilename}`;
          }
