...
export const ServiceSidebar: React.FC<ServiceSidebarProps> = React.memo(({ model, onCTAClick, serviceSlug }) => {
  ...
  return (
    <>
      ...
      >
        <div
          className="bg-gradient-to-br from-primary-600 to-primary-800 rounded-l-lg border-t border-b border-l border-primary-700 h-full flex flex-col"
          style={{
            padding: '1rem',
            height: '100%',
            maxHeight: '100%',
        ...
        >
          ...
          {/* Content Area */}
          <div className="flex-1 flex flex-col min-h-0 overflow-hidden" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            {/* Video Section */}
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-hidden" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              ...
              <div className="flex-1 overflow-hidden" style={{ overflow: 'hidden' }}>
                <ul className="space-y-1.5 pr-1">
                  {model.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-1.5">
                      <svg className="w-3.5 h-3.5 text-green-300 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="mt-3 flex-shrink-0">
            <button
              onClick={onCTAClick}
              className="w-full bg-white hover:bg-gray-50 text-primary-600 font-bold py-2.5 px-3 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg border-2 border-white flex items-center justify-center gap-2 text-sm"
            >
              {model.buttonText}
            </button>
          </div>
