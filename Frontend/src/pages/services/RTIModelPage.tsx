...
export const RTIModelPage: React.FC = () => {
  ...
  const submitApplicationWithoutPayment = async (data: typeof formData) => {
    try {
      ...
      // Fetch state by slug (default to Goa if not found)
      // Default state_id to Goa based on current database
      let stateId = 1; // Goa (update with actual ID from your database)
      try {
        const statesResponse = await statesAPI.getAll() as any;
        if (statesResponse?.success && Array.isArray(statesResponse?.data)) {
          const goaState = statesResponse.data.find((s: any) => s.slug === 'goa');
          if (goaState?.id) {
            stateId = Number(goaState.id);
            console.log(`✅ Found state ID: ${stateId} for Goa`);
          }
        }
      } catch (error) {
        console.warn('⚠️ Could not fetch states from backend, using default ID for Goa:', error);
      }
