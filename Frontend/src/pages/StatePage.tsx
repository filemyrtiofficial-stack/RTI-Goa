...
export const StatePage: React.FC = () => {
  // Try to get state from route param first, then fallback to subdomain, then default to goa
  const { stateSlug } = useParams<{ stateSlug?: string }>();
  const subdomainSlug = getStateSlugFromSubdomain();
  const effectiveSlug = stateSlug || subdomainSlug || 'goa'; // Default to goa if no slug provided
