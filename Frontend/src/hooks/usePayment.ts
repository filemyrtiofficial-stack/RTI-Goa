    ...
    async (
      ...
      try {
        ...
        try {
          ...
          const errorMsg = apiError?.message || 'Failed to create payment order';
          if (errorMsg.includes('Backend server is not running') || errorMsg.includes('Network error')) {
            throw new Error('Payment service is temporarily unavailable. Please ensure the backend server is running or contact support.');
          }
        ...
        await initializePayment(
          ...
          async (response: RazorpayResponse) => {
            try {
              ...
              let verifyResponse;
              try {
                verifyResponse = await paymentsAPI.verifyPayment({
                ...
                // Handle API errors with better messages
                const errorMsg = apiError?.message || 'Payment verification failed';
                if (errorMsg.includes('Backend server is not running') || errorMsg.includes('Network error')) {
                  throw new Error('Payment verification service is temporarily unavailable. Your payment may have been successful. Please contact support with your payment ID: ' + response.razorpay_payment_id);
                }
                throw new Error(errorMsg);
              }
